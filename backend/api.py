...
# ============================================================================
# IBM ADVANCED COMPONENTS ENDPOINTS
# ============================================================================

@app.post("/api/financial-twin/create")
async def create_financial_twin(request: Request):
    """Create Financial Digital Twin using Neo4j + RAG."""
    sessionid = request.cookies.get("sessionid")
    if not sessionid:
        raise HTTPException(status_code=401, detail="Session ID required")
    
    try:
        from ibm_components import get_financial_twin
        twin = get_financial_twin()
        
        # Get user's financial data
        bank_data = await mcp_client.get_bank_transactions(sessionid)
        net_worth = await mcp_client.get_net_worth(sessionid)
        goals = goals_manager.list_goals(sessionid)
        
        financial_data = {
            "accounts": [
                {"id": "acc_001", "type": "savings", "balance": 50000},
                {"id": "acc_002", "type": "current", "balance": 25000}
            ],
            "transactions": bank_data.get('transactions', [])[:100],  # Last 100 transactions
            "goals": goals
        }
        
        result = twin.create_financial_graph(sessionid, financial_data)
        
        return {
            "status": "success",
            "digital_twin": result,
            "technology": "Neo4j Graph Database + watsonx RAG",
            "capabilities": [
        ...
        logger.error(f"Failed to create financial twin: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/financial-twin/what-if")
async def run_what_if_scenario(request: Request):
    """Run what-if scenarios on Financial Digital Twin."""
    sessionid = request.cookies.get("sessionid")
    if not sessionid:
        raise HTTPException(status_code=401, detail="Session ID required")
    ...
    try:
        ...
        result = twin.run_what_if_scenario(sessionid, scenario)
        
        return {
            "status": "success",
            "analysis": result,
            "powered_by": "Neo4j + watsonx AI"
        }
...
@app.post("/api/payment-scheduler/optimize")
async def optimize_payment_schedule(request: Request):
    """Optimize payment schedule using LangGraph autonomous agent."""
    sessionid = request.cookies.get("sessionid")
    if not sessionid:
        raise HTTPException(status_code=401, detail="Session ID required")
    
    try:
        from ibm_components import get_payment_scheduler
        ...
        financial_data = {
            ...
            "current_balance": 75000  # Mock balance
        }
        
        result = await scheduler.schedule_payments(sessionid, financial_data)
        
        return {
            "status": "success",
            "schedule": result,
            "technology": "IBM LangGraph Autonomous Agent"
        }
        
    except Exception as e:
        logger.error(f"Payment scheduling failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/knowledge-augmented/query")
async def knowledge_augmented_query(request: Request):
    """Query using Knowledge-Augmented LLM with Weaviate."""
    sessionid = request.cookies.get("sessionid")
    if not sessionid:
        raise HTTPException(status_code=401, detail="Session ID required")
    ...
    try:
        from ibm_components import get_knowledge_llm
        knowledge_llm = get_knowledge_llm()
        ...
        result = await knowledge_llm.augment_query(sessionid, query, user_context)
        
        return {
            "status": "success",
            "response": result,
            "technology": "watsonx AI + Weaviate Vector Database"
        }
        
    except Exception as e:
        logger.error(f"Knowledge-augmented query failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/risk-guard/start")
async def start_risk_monitoring(request: Request):
    """Start real-time risk monitoring with Event Streams."""
    sessionid = request.cookies.get("sessionid")
    if not sessionid:
        raise HTTPException(status_code=401, detail="Session ID required")
    
    try:
        from ibm_components import get_risk_guard
        risk_guard = get_risk_guard()
        
        result = await risk_guard.start_monitoring(sessionid)
...
@app.get("/api/risk-guard/dashboard")
async def get_risk_dashboard(request: Request):
    ...
    try:
        ...
        dashboard = await risk_guard.get_risk_dashboard(sessionid)
        
        return {
            "status": "success",
            "dashboard": dashboard,
            "technology": "IBM Event Streams + AI Risk Models"
        }
...
@app.get("/api/ibm-components/status")
async def get_ibm_components_status():
    """Get status of all IBM advanced components."""
    try:
        from ibm_components import COMPONENTS
        
        return {
            "status": "active",
            "components": COMPONENTS,
            "integration": {
                "watsonx": os.getenv("LLM_PROVIDER", "google").lower() == "watsonx",
                "neo4j": "ready",
                "weaviate": "ready",
                "event_streams": "ready",
                "langgraph": "ready"
            },
            "message": "All IBM advanced components are integrated and ready"
        }
